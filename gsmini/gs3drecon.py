...
def interpolate_gradients(gx, gy, img, cm, markermask):
    ...
    gx_interpol = griddata(
        ind2,
        gx[(ind2[:, 0], ind2[:, 1])],
        gx[(ind2not[:, 0], ind2not[:, 1])],
        method="nearest",
    )
    gx[(ind2not[:, 0], ind2not[:, 1])] = gx_interpol
    gy_interpol = griddata(
        ind2,
        gy[(ind2[:, 0], ind2[:, 1])],
        gy[(ind2not[:, 0], ind2not[:, 1])],
        method="nearest",
    )
...
def demark(gx, gy, markermask):
    gx_interp = interpolate_grad(gx, markermask)
    gy_interp = interpolate_grad(gy, markermask)
...
def poisson_dct_neumaan(gx, gy):
    ...
    b = np.zeros(gx.shape, dtype=np.float32)
    b[0, 1:-2] = -gy[0, 1:-2]
    b[-1, 1:-2] = gy[-1, 1:-2]
    b[1:-2, 0] = -gx[1:-2, 0]
    b[1:-2, -1] = gx[1:-2, -1]
...
class Reconstruction3D:
    def __init__(self, imgw, imgh):
        ...
        self.dm_zero = np.zeros((imgh, imgw), dtype=np.float32)
    ...
    def load_nn(self, net_path, cpuorgpu):
        ...
        if cpuorgpu == "cuda":
            ...
            checkpoint = torch.load(
                net_path, map_location=lambda storage, loc: storage.cuda(0)
            )
    ...
    def get_depthmap(self, frame, mask_markers, cm=None):
        ...
        ##################################################################
        if cm is None:
            cm, _ = np.ones(frame.shape[:2]), np.where(np.ones(frame.shape[:2]))
        ...
        nx = np.zeros(frame.shape[:2], dtype=np.float32)
        ny = np.zeros(frame.shape[:2], dtype=np.float32)
        dm = np.zeros(frame.shape[:2], dtype=np.float32)
        ...
        nx[inds] = out[:, 0].cpu().detach().numpy()
        ny[inds] = out[:, 1].cpu().detach().numpy()
