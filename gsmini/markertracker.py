...
class MarkerTracker:
    def __init__(self, marker_image, params=None):
        ...
        if self.params["DoPlot"]:
            for i in range(len(new_centers)):
                p = new_centers[i, :]
                cv2.circle(
                    img, (int(p[1]), int(p[0])), radius=int(radii[i]), color=(0, 255, 0)
                )
    ...
    def track_markers(self, frame):
        ...
        if ydim != self.ydim or xdim != self.xdim:
            print(
                "image size {xdim} x {ydim} differs from model size {self.xdim} x {self.ydim}"
            )
        ...
        if settings["toplot"] == 2:
            ...
            cv2.imshow(
                "Marker Frame",
                cv2.resize(frame, (2 * frame.shape[1], 2 * frame.shape[0])),
            )
            cv2.waitKey(5)
